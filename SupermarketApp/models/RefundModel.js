...
function RefundModel() {
  ...
  return {
    createRefund(orderId, amount, reason, callback) {
      ensureTable((err) => {
        if (err) return callback(err);
        const sql = 'INSERT INTO refunds (order_id, amount, reason, status) VALUES (?, ?, ?, ?)';
        db.query(sql, [orderId, amount, reason, 'pending'], callback);
      });
    },
    ...
    getRefundsByOrder(orderId, callback) {
      ensureTable((err) => {
        if (err) return callback(err);
        const sql = 'SELECT * FROM refunds WHERE order_id = ? ORDER BY created_at DESC';
